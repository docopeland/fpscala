...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors filterNot (explored contains _._1) to(LazyList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, move) #:: others => {
      val next = newNeighborsOnly(neighborsWithHistory(block, move), explored)
      (block, move) #:: from(others ++ next, explored + block)
    }

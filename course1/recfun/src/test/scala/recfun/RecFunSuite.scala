...
class RecFunSuite {
  ...
  @Test def `balance: '(if (zero? x) max (/ 1 x))' is balanced`: Unit =
    assertEquals(true,balance("(if (zero? x) max (/ 1 x))".toList))

  @Test def `balance: 'I told him ...' is balanced`: Unit =
    assertEquals(true,balance("I told him (that it's not (yet) done).\n(But he wasn't listening)".toList))

  @Test def `balance: ':-)' is unbalanced`: Unit = {
    assertEquals(false, balance(":-)".toList))
  }
  ...
  @Test def `pascal: col=4,row=8`: Unit =
